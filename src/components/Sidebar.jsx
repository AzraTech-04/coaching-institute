...
const navItems = [
  ...
  {
    type: 'group', name: 'Students', icon: 'students',
    children: [
  ...
  {
    type: 'group', name: 'Admissions', icon: 'admissions',
    children: [
  ...
  {
    type: 'group', name: 'Academics', icon: 'academics',
    children: [
  ...
  {
    type: 'group', name: 'Analytics', icon: 'analytics',
    children: [
  ...
  {
    type: 'group', name: 'Communication', icon: 'communication',
    children: [
  ...
  {
    type: 'group', name: 'Settings', icon: 'settings',
    children: [
...
const iconPaths = {
  ...
  admissions: 'M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2m14-14a4 4 0 11-8 0 4 4 0 018 0zM19 8v6m3-3h-6',
  academics: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253',
  analytics: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2',
  communication: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.86 9.86 0 01-4-.8L3 20l1.3-3.9A7.96 7.96 0 013 12c0-4.418 4.03-8 9-8s9 3.582 9 8z',
...
function Icon({ name, className }) {
  return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d={iconPaths[name]} />
    </svg>
  )
}
...
function Sidebar({ isOpen, onClose }) {
  ...
  const visibleNavItems =
    role && rolePermissions[role] ? filterNavItems(navItems, rolePermissions[role]) : navItems

  // Auto-expand whichever group contains the current page, whenever the route changes.
  useEffect(() => {
    const activeGroup = visibleNavItems.find(
      (item) => item.type === 'group' && item.children.some((c) => c.path === location.pathname)
    )
    if (activeGroup) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:static top-0 left-0 h-screen w-64 bg-white border-r border-neutral-200 flex flex-col z-40 transform transition-transform duration-200 ${
      ...
      >
        ...
        <nav className="flex-1 min-h-0 overflow-y-auto px-3 py-4 space-y-1">
          {visibleNavItems.map((item) => {
            ...
            const isGroupActive = item.children.some((c) => c.path === location.pathname)
              ...
              <div key={item.name}>
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                    isGroupActive ? 'text-brand-700' : 'text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900'
                  }`}
                >
                ...
                {isGroupOpen && (
                  <div className="mt-1 ml-4 pl-3 border-l border-neutral-200 space-y-0.5">
                    {item.children.map((child) => (
                      <NavLink key={child.path} to={child.path} onClick={onClose} className={childLinkClasses}>
                        {child.name}
                      </NavLink>
                    ))}
                  </div>
