...
function SmsEmail() {
  const [messages, setMessages] = useState(initialMessages.filter((m) => m.channel === 'SMS' || m.channel === 'Email'))
  ...
  const stats = [
    { label: 'Total Messages', value: messages.length, change: 'SMS + Email', trend: 'up' },
    { label: 'SMS Sent', value: messages.filter((m) => m.channel === 'SMS' && m.status === 'Sent').length, change: 'Delivered', trend: 'up' },
    { label: 'Email Sent', value: messages.filter((m) => m.channel === 'Email' && m.status === 'Sent').length, change: 'Delivered', trend: 'up' },
    { label: 'Scheduled + Drafts', value: messages.filter((m) => m.status !== 'Sent').length, change: 'Pending', trend: 'down' },
  ]
  ...
  return (
    <div>
      <PageHeader
        title="SMS & Email"
        subtitle="Reach students, parents, and faculty over SMS and Email."
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-4">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search messages..." className="w-full pl-9 pr-3 py-2.5 text-sm border border-neutral-300 rounded-lg outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500" />
        </div>
        <select value={channelFilter} onChange={(e) => setChannelFilter(e.target.value)} className="px-3.5 py-2.5 text-sm border border-neutral-300 rounded-lg outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 bg-white">
      ...
      <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
        {messages.length === 0 ? (
          <EmptyState title="No SMS or Email messages yet" message="Compose your first message to reach students or faculty." actionLabel="+ Compose Message" onAction={() => setModalOpen(true)} />
        ) : filtered.length === 0 ? (
      ...
      <ComposeMessageModal open={modalOpen} onClose={() => setModalOpen(false)} onSubmit={handleCompose} modalTitle="Compose SMS or Email" channelOptions={['SMS', 'Email']} />
