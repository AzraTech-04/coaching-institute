...
function Notifications() {
  const [messages, setMessages] = useState(initialMessages.filter((m) => m.channel === 'Notification'))
  ...
  const stats = [
    { label: 'Total Notifications', value: messages.length, change: 'All time', trend: 'up' },
    { label: 'Sent', value: messages.filter((m) => m.status === 'Sent').length, change: 'Delivered', trend: 'up' },
    { label: 'Scheduled', value: messages.filter((m) => m.status === 'Scheduled').length, change: 'Upcoming', trend: 'up' },
    { label: 'Drafts', value: messages.filter((m) => m.status === 'Draft').length, change: 'Not yet sent', trend: 'down' },
  ]
  ...
  return (
    <div>
      <PageHeader
        title="Notifications"
        subtitle="In-app notifications sent to students, faculty, and leads."
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-4">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search notifications..." className="w-full pl-9 pr-3 py-2.5 text-sm border border-neutral-300 rounded-lg outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500" />
        </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
        {messages.length === 0 ? (
          <EmptyState title="No notifications yet" message="Create your first notification to reach students or faculty." actionLabel="+ Create Notification" onAction={() => setModalOpen(true)} />
        ) : filtered.length === 0 ? (
          <EmptyState title="No matching notifications" message="Try adjusting your search or filters." />
        ) : (
      ...
      <ComposeMessageModal open={modalOpen} onClose={() => setModalOpen(false)} onSubmit={handleCompose} modalTitle="Create Notification" channelOptions={['Notification']} />
